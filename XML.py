...
#-*- coding: utf8 -*-
## --------------Change log-----------------------
## 2022/1/3 DB2/EDB加上db type、db account(line84, 120)
## 2022/1/3 掃描選項加上全掃所有Job(line222)
## 2022/1/4 file加上尋找.REJ檔案(Reject file) (line143)
## (在Source Target都有同一張表,且有Reject file之來源為偽)
## ------------------------------------------------
...
def Record_extract(job, src):
    ...
    # Job 往下找 Property = OutputPins 的 Record
    for r in job.findall(".//Property[@Name='OutputPins']/.."):
        # Record 往下確認 Property StageType = src
        for p in r.findall(".//Property[@Name='StageType']"):
            if  src in p.text:
                Out_rcrd.append(r)

    # Job 往下找 Property = InputPins 的 Record
...
if __name__ == "__main__":  
    ...
    # Job List
    Job = input('請輸入以半形逗號分隔之程式清單 or *(全部的job都要掃):')
